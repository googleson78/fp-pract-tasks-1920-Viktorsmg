...
main = do 
    ...
    BL.writeFile "render.bmp" 
        (seq faces_cornell_mirror (BL.pack (makeBMP ((floor width), (floor height)) 
            (traceManySeededSPPThreaded 10 faces_cornell_mirror (width, height, (degToRad (fov / 2.0))) (Ray campos (normalize camdir)) samples gen) 
            )))
